...
void makeG1010(coord point[], vector<int> voisin[10]){
	int k = 0;
	for (int i = 0; i < 10; ++i)
	{
		for (int j = 0; j < 10; ++j)
	...
	for (int i = 0; i < 10; ++i)
	{
		...
		{
			int k = i * 9 + j;
			voisin[k].push_back((i + 1) * 9 + j);
			voisin[k].push_back(i * 9 + (j + 1));
			voisin[k].push_back((i + 1) * 9 + (j + 1));
		}
	}
...
int main(int argc, char* argv[]){
	...
	if(argc >= 3){
		...
		bool printall = false;
		int n = 10 * 10;//= atoi(argv[1]);                   //Le nombre de points.
...
*/
		...
		time = clock();
		dijkstra(10*10, voisin, point, pere);
		time = clock() - time;
		if(printall){
			cout << "pere = ";
			printarray(n, pere);
...
float distance(coord* a, coord* b){
	return pow(abs(b->abs - a->abs) + abs(b->ord - a->ord), 2);//pow(pow(b->abs - a->abs , 2) + pow(b->ord - a->ord , 2), (float)1/2);
}
